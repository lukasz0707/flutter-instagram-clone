...
const schema = Schema([
  Table(
    ...
    // Added index for email
    indexes: [
      Index('email_idx', [IndexedColumn('email')]), // Renamed index for clarity
    ],
  ),
  Table(
    'posts',
  ...
  Table(
    ...
    indexes: [
      ...
      // Added compound indexes for likes for performance
      Index('user_comment_idx', [
        IndexedColumn('user_id'),
        IndexedColumn('comment_id'),
      ]),
      Index('user_post_idx', [
        IndexedColumn('user_id'),
        IndexedColumn('post_id'),
      ]),
    ],
  ),
  Table(
  ...
  Table(
    ...
    indexes: [
      ...
      Index('user', [IndexedColumn('user_id')]),
      // Added compound index for participants for performance
      Index('user_conversation_idx', [
        IndexedColumn('user_id'),
        IndexedColumn('conversation_id'),
      ]),
    ],
  ),
  Table(
  ...
  Table(
    ...
    indexes: [
      Index(
        'user', // This index still makes sense for performance
